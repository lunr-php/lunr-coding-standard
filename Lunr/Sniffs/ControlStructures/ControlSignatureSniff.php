...
class ControlSignatureSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        if (in_array($tokens[$stackPtr]['code'], [ T_ELSE, T_TRY ]) && $found !== 'newline' && $isAlternative === false) {
            ...
            $fix = $phpcsFile->addFixableError($error, $stackPtr, 'NewlineAfterElse', $data);
            if ($fix === true) {
                $phpcsFile->fixer->beginChangeset();
                for ($i = ($stackPtr + 1); $i <= $stackPtr + $found; $i++) {
        ...
        // Newline after closing parenthesis.
        if (isset($tokens[$stackPtr]['parenthesis_closer']) === true
            ...
            if ($found !== $expected) {
                if ($isAlternative === true) {
                    ...
                    $error = 'Expected %s after closing parenthesis; found %s space(s)';
                }
                $data  = [
                    $expected,
                    $found,
                ...
                if ($fix === true) {
                    if ($isAlternative) {
                        ...
                        $fix   = $phpcsFile->addFixableError($error, $opener, 'NewlineAfterCloseParenthesis', $data);
                        if ($fix === true) {
                            $phpcsFile->fixer->beginChangeset();
                            for ($i = ($closer + 1); $i < $opener; $i++) {
                                if (trim($tokens[$i]['content']) !== '') {
        ...
        if ($found !== 'newline') {
            $error = 'Expected newline after closing brace; %s space(s) found';
            $data  = [$found];
            ...
            if ($phpcsFile->findNext(Tokens::$commentTokens, ($closer + 1), $stackPtr) !== false) {
                ...
                $phpcsFile->addError($error, $closer, 'NewlineAfterCloseBrace', $data);
                return;
            }
